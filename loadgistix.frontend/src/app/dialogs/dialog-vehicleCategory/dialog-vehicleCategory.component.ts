...
export class DialogVehicleCategoryComponent implements OnDestroy {
    ...
    async initDelete(id: any) {
        ...
        confirmation.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe((result) => {
            if (result === 'confirmed') {
                ...
                this.sqlService.deleteItem('vehicleCategories', id).pipe(takeUntil(this._unsubscribeAll)).subscribe(() => {
