...
@Component({
    ...
    standalone: true,
    imports: [NgClass, NgFor, NgForOf, NgIf, MatIconModule, MatSlideToggleModule, MatSelectModule, MatOptionModule, MatSnackBarModule, MatProgressSpinnerModule, MatDialogModule, MatDatepickerModule, MatButtonModule, FormsModule, ReactiveFormsModule, MatFormFieldModule, MatInputModule, SortPipe],
    encapsulation: ViewEncapsulation.None
})
export class DialogLoadTypeComponent implements OnDestroy {
    ...
    async initDelete(id: any) {
        ...
        confirmation.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe((result) => {
            if (result === 'confirmed') {
                ...
                this.sqlService.deleteItem('loadTypes', id).pipe(takeUntil(this._unsubscribeAll)).subscribe(() => {
