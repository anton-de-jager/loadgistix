...
@Component({
    selector: 'dialog-make',
    templateUrl: 'dialog-make.component.html',
    standalone: true,
...
export class DialogMakeComponent implements OnDestroy {
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<DialogMakeComponent>,
    ...
    async initDelete(id: any) {
        ...
        confirmation.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe((result) => {
            if (result === 'confirmed') {
                ...
                this.sqlService.deleteItem('makes', id).pipe(takeUntil(this._unsubscribeAll)).subscribe(() => {
