...
import { DialogReturnReasonComponent } from "app/dialogs/dialog-returnReason/dialog-returnReason.component";
...
@Component({
    selector: 'lookups-returnReason',
    templateUrl: './returnReason.component.html',
...
export class LookupsReturnReasonComponent implements OnInit, OnDestroy {
    ...
    getReturnReasons() {
        this.sqlService.getItems('returnReasons').pipe(takeUntil(this._unsubscribeAll)).subscribe(apiResult => {
            if (apiResult.data == "Unauthorised") {
                this.router.navigate(['/sign-out']);;
            } else {
                this.returnReasonList = apiResult.data;
                this.dataSource = new MatTableDataSource(this.returnReasonList);
                this.iPaginator = 0;
                this.setPaginator();
            }
        })
    ...
    initUpsertReturnReason(row: any) {
        ...
        const dialogRef = this.dialog.open(DialogReturnReasonComponent,
            dialogConfig);
        ...
        dialogRef.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe(result => {
            if (result !== false) {
                switch (result.action) {
                    case 'insert':
                        this.sqlService.createItem('returnReasons', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const newItem = extractApiData(apiResult.data);
                            if (newItem) {
                                this.returnReasonList.push(newItem);
                                this.dataSource = new MatTableDataSource(this.returnReasonList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'update':
                        this.sqlService.updateItem('returnReasons', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const updatedItem = extractApiData(apiResult.data);
                            const existingItem = this.returnReasonList.find(x => x.id == result.value.id);
                        ...
                        break;
                    case 'delete':
                        this.returnReasonList = this.returnReasonList.filter(x => x.id != result.value);
                        this.dataSource = new MatTableDataSource(this.returnReasonList);
                        this.iPaginator = 0;
                        this.setPaginator();
                        break;
