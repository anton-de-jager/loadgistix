...
import { DialogStockProblemComponent } from "app/dialogs/dialog-stockProblem/dialog-stockProblem.component";
...
@Component({
    selector: 'lookups-stockProblem',
    templateUrl: './stockProblem.component.html',
...
export class LookupsStockProblemComponent implements OnInit, OnDestroy {
    ...
    dataSource!: MatTableDataSource<stockProblem>;
    form!: FormGroup;
    ...
    getStockProblems() {
        this.sqlService.getItems('stockProblems').pipe(takeUntil(this._unsubscribeAll)).subscribe(apiResult => {
            if (apiResult.data == "Unauthorised") {
                this.router.navigate(['/sign-out']);;
            } else {
                this.stockProblemList = apiResult.data;
                this.dataSource = new MatTableDataSource(this.stockProblemList);
                this.iPaginator = 0;
                this.setPaginator();
            }
        })
    ...
    initUpsertStockProblem(row: any) {
        ...
        const dialogRef = this.dialog.open(DialogStockProblemComponent,
            dialogConfig);
        ...
        dialogRef.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe(result => {
            if (result !== false) {
                switch (result.action) {
                    case 'insert':
                        this.sqlService.createItem('stockProblems', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const newItem = extractApiData(apiResult.data);
                            if (newItem) {
                                this.stockProblemList.push(newItem);
                                this.dataSource = new MatTableDataSource(this.stockProblemList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'update':
                        this.sqlService.updateItem('stockProblems', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const updatedItem = extractApiData(apiResult.data);
                            const existingItem = this.stockProblemList.find(x => x.id == result.value.id);
                        ...
                        break;
                    case 'delete':
                        this.stockProblemList = this.stockProblemList.filter(x => x.id != result.value);
                        this.dataSource = new MatTableDataSource(this.stockProblemList);
                        this.iPaginator = 0;
                        this.setPaginator();
                        break;
