...
import { DialogVehicleTypeComponent } from "app/dialogs/dialog-vehicleType/dialog-vehicleType.component";
...
export class LookupsVehicleTypeComponent implements OnInit, OnDestroy {
    ...
    displayedColumns: string[] = ['vehicleCategoryDescription', 'description'];
        ...
        dialogRef.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe(result => {
            if (result !== false) {
                switch (result.action) {
                    case 'insert':
                        this.sqlService.createItem('vehicleTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const newItem = extractApiData(apiResult.data);
                            if (newItem) {
                                this.vehicleTypeList.push(newItem);
                                this.dataSource = new MatTableDataSource(this.vehicleTypeList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'update':
                        this.sqlService.updateItem('vehicleTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const updatedItem = extractApiData(apiResult.data);
                            const existingItem = this.vehicleTypeList.find(x => x.id == result.value.id);
                            if (existingItem && updatedItem) {
                                existingItem.description = updatedItem.description;
                                existingItem.vehicleCategoryId = updatedItem.vehicleCategoryId;
                                existingItem.vehicleCategoryDescription = updatedItem.vehicleCategoryDescription;
                                this.dataSource = new MatTableDataSource(this.vehicleTypeList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'delete':
                        this.vehicleTypeList = this.vehicleTypeList.filter(x => x.id != result.value);
                        this.dataSource = new MatTableDataSource(this.vehicleTypeList);
                        this.iPaginator = 0;
                        this.setPaginator();
                        break;
