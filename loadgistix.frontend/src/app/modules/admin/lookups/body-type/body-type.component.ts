...
import { DialogBodyTypeComponent } from "app/dialogs/dialog-bodyType/dialog-bodyType.component";
...
export class LookupsBodyTypeComponent implements OnInit, OnDestroy {
    ...
    dataSource!: MatTableDataSource<bodyType>;
    form!: FormGroup;
    ...
    getBodyTypes() {
        this.sqlService.getItems('bodyTypes').pipe(takeUntil(this._unsubscribeAll)).subscribe(apiResult => {
            if (apiResult.data == "Unauthorised") {
                this.router.navigate(['/sign-out']);;
            } else {
                this.bodyTypeList = apiResult.data;
                this.dataSource = new MatTableDataSource(this.bodyTypeList);
                this.iPaginator = 0;
                this.setPaginator();
            }
        })
    ...
    initUpsertBodyType(row: any) {
        ...
        const dialogRef = this.dialog.open(DialogBodyTypeComponent,
            dialogConfig);
        ...
        dialogRef.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe(result => {
            if (result !== false) {
                switch (result.action) {
                    case 'insert':
                        this.sqlService.createItem('bodyTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const newItem = extractApiData(apiResult.data);
                            if (newItem) {
                                this.bodyTypeList.push(newItem);
                                this.dataSource = new MatTableDataSource(this.bodyTypeList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'update':
                        this.sqlService.updateItem('bodyTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const updatedItem = extractApiData(apiResult.data);
                            const existingItem = this.bodyTypeList.find(x => x.id == result.value.id);
                        ...
                        break;
                    case 'delete':
                        this.bodyTypeList = this.bodyTypeList.filter(x => x.id != result.value);
                        this.dataSource = new MatTableDataSource(this.bodyTypeList);
                        this.iPaginator = 0;
                        this.setPaginator();
                        break;
