...
import { DialogLicenceTypeComponent } from "app/dialogs/dialog-licenceType/dialog-licenceType.component";
...
export class LookupsLicenceTypeComponent implements OnInit, OnDestroy {
    ...
    dataSource!: MatTableDataSource<licenceType>;
    form!: FormGroup;
    ...
    getLicenceTypes() {
        this.sqlService.getItems('licenceTypes').pipe(takeUntil(this._unsubscribeAll)).subscribe(apiResult => {
            if (apiResult.data == "Unauthorised") {
                this.router.navigate(['/sign-out']);;
            } else {
                this.licenceTypeList = apiResult.data;
                this.dataSource = new MatTableDataSource(this.licenceTypeList);
                this.iPaginator = 0;
                this.setPaginator();
            }
        })
        ...
        dialogRef.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe(result => {
            if (result !== false) {
                switch (result.action) {
                    case 'insert':
                        this.sqlService.createItem('licenceTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const newItem = extractApiData(apiResult.data);
                            if (newItem) {
                                this.licenceTypeList.push(newItem);
                                this.dataSource = new MatTableDataSource(this.licenceTypeList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'update':
                        this.sqlService.updateItem('licenceTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            const updatedItem = extractApiData(apiResult.data);
                            const existingItem = this.licenceTypeList.find(x => x.id == result.value.id);
                        ...
                        break;
                    case 'delete':
                        this.licenceTypeList = this.licenceTypeList.filter(x => x.id != result.value);
                        this.dataSource = new MatTableDataSource(this.licenceTypeList);
                        this.iPaginator = 0;
                        this.setPaginator();
                        break;
