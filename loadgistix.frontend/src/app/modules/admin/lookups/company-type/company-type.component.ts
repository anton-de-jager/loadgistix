...
import { DialogCompanyTypeComponent } from "app/dialogs/dialog-companyType/dialog-companyType.component";
...
export class LookupsCompanyTypeComponent implements OnInit, OnDestroy {
    ...
    getCompanyTypes() {
        this.sqlService.getItems('companyTypes').pipe(takeUntil(this._unsubscribeAll)).subscribe(apiResult => {
            if (apiResult.data == "Unauthorised") {
                this.router.navigate(['/sign-out']);;
            } else {
                this.companyTypeList = apiResult.data;
                this.dataSource = new MatTableDataSource(this.companyTypeList);
                this.iPaginator = 0;
                this.setPaginator();
            }
        })
    ...
    initUpsertCompanyType(row: any) {
        ...
        const dialogRef = this.dialog.open(DialogCompanyTypeComponent,
            dialogConfig);
        ...
        dialogRef.afterClosed().pipe(takeUntil(this._unsubscribeAll)).subscribe(result => {
            if (result !== false) {
                switch (result.action) {
                    case 'insert':
                        this.sqlService.createItem('companyTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            // Handle both array and object responses
                            const newItem = Array.isArray(apiResult.data) ? extractApiData(apiResult.data) : apiResult.data;
                            if (newItem) {
                                this.companyTypeList.push(newItem);
                                this.dataSource = new MatTableDataSource(this.companyTypeList);
                                this.iPaginator = 0;
                                this.setPaginator();
                            }
                        ...
                        break;
                    case 'update':
                        this.sqlService.updateItem('companyTypes', result.value).pipe(takeUntil(this._unsubscribeAll)).subscribe((apiResult) => {
                            // Handle both array and object responses
                            const updatedItem = Array.isArray(apiResult.data) ? extractApiData(apiResult.data) : apiResult.data;
                            const existingItem = this.companyTypeList.find(x => x.id == result.value.id);
                        ...
                        break;
                    case 'delete':
                        this.companyTypeList = this.companyTypeList.filter(x => x.id != result.value);
                        this.dataSource = new MatTableDataSource(this.companyTypeList);
                        this.iPaginator = 0;
                        this.setPaginator();
                        break;
